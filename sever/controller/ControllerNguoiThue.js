const Renter = require("../model/NguoiThueTro");
const Room = require("../model/PhongTro");
const changeRoomStatus = (renter) => {
  return renter > 0 ? "da thue" : "con trong";
};
const getAllRenters = async (req, res) => {
  const Renters = await Renter.find();
  if (!Renters) return res.status(204).json({ message: "khong tim thay phong" });
  res.json(Renters);
};
